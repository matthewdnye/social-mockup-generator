...
const FacebookPost = dynamicImport(
  () => import('@/components/mockups/FacebookPost').then((mod) => mod.FacebookPost),
  {
    ssr: false,
    loading: () => (
      <div className="w-[500px] h-[300px] flex items-center justify-center bg-white rounded-xl">
...
const FacebookEditor = dynamicImport(
  () => import('@/components/editor/FacebookEditor').then((mod) => mod.FacebookEditor),
  {
    ssr: false,
    loading: () => (
      ...
      <header className="border-b bg-white px-4 py-3">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-lg font-semibold text-gray-900">Facebook Post Generator</h1>
              <p className="text-sm text-gray-600">
                Create realistic Facebook post mockups
              </p>
            </div>
          </div>
          <div className="flex items-center">
            <ResetButton />
            <div className="ml-3">
              <ExportOptions targetRef={postRef} filename="facebook-mockup" />
            </div>
          </div>
        </div>
      </header>
